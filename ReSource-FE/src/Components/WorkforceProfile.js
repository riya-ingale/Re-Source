...
export default function WorkforceProfile() {
  ...
  useEffect(() =>{
    fetch("http://127.0.0.1:8000/institute/workforce_profile/"+sessionStorage.getItem('user_id')+"/"+r_id+"/"+l_id)
  ...
  const handleLab = (e,p) =>{
    setL_id(p);
    fetch("http://127.0.0.1:8000/institute/workforce_profile/"+sessionStorage.getItem('user_id')+"/"+r_id+"/"+p)
    .then(response=>response.json())
    .then(body=>
      {
        setRes(body);
  ...
  const handleRes = (e,p) =>{
    setR_id(p);
    fetch("http://127.0.0.1:8000/institute/workforce_profile/"+sessionStorage.getItem('user_id')+"/"+p+"/"+l_id)
    .then(response=>response.json())
    .then(body=>
      {
        setRes(body);
    ...
    <div className='container profile-container'>
      ...
      <div className='row'>
        ...
        <div className="card profilecards">
            ...
            {res.workforce_data.city?<li className="mb-2"><strong className='strlist'>City: </strong> {res.workforce_data.city}</li>:<div></div>}
            ...
            {res.workforce_data.phone_no?<li className="mb-2"><strong className='strlist'>Phone Number: </strong>{res.workforce_data.phone_no}</li>:<div></div>}
              ...
              <li className="mb-2"><strong className='strlist'>Position: </strong>{res.workforce_data.position}</li>
            </ul>
            </div>
            </div>
        </div>
